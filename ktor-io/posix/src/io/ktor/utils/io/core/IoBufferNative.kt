...
public actual class IoBuffer actual constructor(
    ...
    public constructor(content: CPointer<ByteVar>, contentCapacity: Int) : this(
        Memory.of(content, contentCapacity),
        null,
        NoPool as ObjectPool<ChunkBuffer>
    )

    override val endOfInput: Boolean get() = !canRead()

    internal actual override val pool: ObjectPool<ChunkBuffer>
    ...
    @Deprecated("Binary compatibility.", level = DeprecationLevel.HIDDEN)
    override fun writeFully(src: CPointer<ByteVar>, offset: Int, length: Int) {
        (this as Buffer).writeFully(src, offset, length)
    }

    @Deprecated("Binary compatibility.", level = DeprecationLevel.HIDDEN)
    override fun writeFully(src: CPointer<ByteVar>, offset: Long, length: Long) {
    ...
    override fun duplicate(): IoBuffer = (origin ?: this).let { newOrigin ->
        newOrigin.acquire()
        IoBuffer(memory, newOrigin, pool).also { copy ->
            duplicateTo(copy)
        }
    }
    ...
    public actual companion object {
        ...
        public actual val Empty: IoBuffer = IoBuffer(Memory.Empty, null, EmptyBufferPoolImpl as ObjectPool<ChunkBuffer>)
        ...
        public actual val NoPool: ObjectPool<IoBuffer> = object : NoPoolImpl<IoBuffer>() {
            override fun borrow(): IoBuffer {
                return IoBuffer(DefaultAllocator.alloc(DEFAULT_BUFFER_SIZE), null, this as ObjectPool<ChunkBuffer>)
            }
...
public fun ChunkBuffer(ptr: CPointer<*>, lengthInBytes: Int, origin: ChunkBuffer?): ChunkBuffer {
    val pool = origin?.pool ?: (IoBuffer.NoPool as ObjectPool<ChunkBuffer>)
    return IoBuffer(Memory.of(ptr, lengthInBytes), origin, pool)
}
